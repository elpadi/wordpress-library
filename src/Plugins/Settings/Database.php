...
class Database {
	...
	public function __construct($filepath, $tables=[]) {
		...
		$this->tables = $tables;
		foreach ($tables as $name => $schema) $this->initTable($name, $schema);
	}
	...
	public function query($sql, $stmtProcessor=null) {
		foreach (array_filter(array_keys($this->tables), function($name) use ($sql) { return strpos($sql, $name); }) as $name) {
	...
	public function select($sql, $stmtProcessor=null, $iterator=null, $col=null) {
		if ($result = $this->query($sql, $stmtProcessor)) {
			...
			while ($row = $result->fetchArray()) {
				...
				if ($iterator) call_user_func($iterator, $val, $index);
				else $values[] = $val;
				$index++;
			}
			return $values;
	...
	public function deleteRow(string $table, $where=[]) {
		return $this->query(sprintf("DELETE FROM %s WHERE %s",
				$table,
				empty($where) ? '1' : implode(', ', F\map(F\pluck($where, 'name'), function($column) { return sprintf('%s=:%s', $column, $column); }))
			), function(&$stmt) use ($where) {
				foreach ($where as $column) static::bindColumnValue($stmt, $column);
			}
		);
	}

	public function updateRow(string $table, array $updates, $where=[]) {
		return $this->query(sprintf("UPDATE %s SET %s WHERE %s",
				$table,
				implode(', ', F\map(F\pluck($updates, 'name'), function($column) { return sprintf('%s=:%s', $column, $column); })),
				empty($where) ? '1' : implode(' AND ', F\map(F\pluck($where, 'name'), function($column) { return sprintf('%s=:%s', $column, $column); }))
			), function(&$stmt) use ($updates, $where) {
				foreach ($updates as $column) static::bindColumnValue($stmt, $column);
				foreach ($where as $column) static::bindColumnValue($stmt, $column);
			}
		);
	}
	...
	public function insertRow(string $table, array $row) {
		...
		return $this->query(sprintf("INSERT OR REPLACE INTO %s (%s) VALUES (%s)",
				$table,
				implode(', ', $columns),
				implode(', ', F\map($columns, function($c) { return ':'.$c; }))
			), function(&$stmt) use ($row) {
				foreach ($row as $column) static::bindColumnValue($stmt, $column);
			}
		);
	}
