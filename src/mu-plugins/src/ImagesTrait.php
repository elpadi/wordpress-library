...
trait ImagesTrait
{
    ...
    {
        return apply_filters('responsive_widths', array_map(function ($s) {
            return intval($s);
        }, explode(',', self::RESPONSIVE_WIDTHS)));
    }
    ...
    protected function getResponsiveAttributesFromPaths($paths_by_width)
    {
        ...
        foreach ($new_path_by_widths as $width => $path) {
            $srcs[] = "$path {$width}w";
        }
    ...
    public function acfGallery($name, $print = true, $gallery_title = '')
    {
        ...
        if (empty($gallery_title)) {
            $gallery_title = get_the_title($post);
        }
