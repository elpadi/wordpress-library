...
abstract class Frontend {
	...
	public static function instance() {
		if (!isset(static::$_instance)) throw new \BadMethodCallException("Instance must first be created.");
		return static::$_instance;
	}

	protected function __construct($dir) {
		$this->dir = $dir;
		if (!defined('THEME_NAME')) define('THEME_NAME', basename($dir));
	...
	public function getAssetContents(string $path) {
		if (strpos($path, './') !== FALSE) throw new \InvalidArgumentException("Path must not contain directory dots.");
		if (strpos($path, '/') === 0) throw new \InvalidArgumentException("Path cannot begin with a slash.");

		$filename = "$this->dir/assets/$path";
		if (!is_readable($filename)) throw new \InvalidArgumentException("Could not find the specified file.");
	...
	public static function ajaxActionJSON($tag, $fn, $errorMsg='') {
		static::ajaxAction($tag, function() use ($fn) {
			...
			try {
				...
				if ($data !== FALSE) wp_send_json_success($data);
			}
			catch (\Exception $e) {
	...
	public static function transientCache($key, $setter, $ttl) {
		if ($value = get_transient($key)) return $value;
