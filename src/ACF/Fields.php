...
abstract class Fields {
	...
	protected function getSelectClauses() {
		...
		return array_merge(
			["`$wpdb->posts`.`ID` AS id"],
			F\map(['title','type','status','date'], function($s) use ($wpdb) { return "`$wpdb->posts`.`post_$s` AS $s"; }),
			F\map(['slug' => 'name'], function($src, $tgt) use ($wpdb) { return "`$wpdb->posts`.`post_$src` AS $tgt"; }),
	...
	protected function getFieldsWhereClause() {
		...
		$fieldNames = F\map($this->getFieldNames(), function($s) { return "'$s'"; });
	...
	public function getPosts() {
		...
		foreach ($posts as &$p) $this->processPostObject($p);
		return $posts;
	}
