...
class Modules extends \ArrayObject {
	...
	public function __construct(Notices $notices, Cover $cover) {
		...
		foreach (['add','edit','delete'] as $s) add_action("wp_ajax_{$s}_tome_module", [$this, $s]);
	...
	public function createPageNavLink($page, $className, $label) {
		return sprintf('<a class="%s" href="%s"><span class="label">%s</span><span class="title">%s</span></a>',
			$className,
	...
	public function updateModulePageQueryVars($query) {
		if ($query->is_main_query()) {
			if (!empty($query->query['name']) && !empty($query->query['tome_module'])) {
				add_filter('redirect_canonical', function($redirect_url) { return ''; });
			...
			if ($this->selected == NULL) $this->selected = $this->getTermBySlug('global');
	...
	protected function fetchTerms() {
		...
		foreach ($terms as $t) $this->addTermRewriteRules($t);
	...
	public function onPostInserted($post, $request, $wasCreated) {
		if ($this->selected) $this->addPost($post->ID);
	}
	...
	public function select() {
		...
		else $this->notices->flash->add('error', sprintf('Could not find module "%s".', $_POST['module-title']));
	...
	public function getPosts($postTypes, $skipSort=false) {
		...
		if ($this->selected && $this->selected->slug != 'global' && F\every($postTypes, [$this, 'hasPostTypeSupport'])) $args['tax_query'] = [
			[
		...
		if ($skipSort == false && count($postTypes) == 1 && $postTypes[0] == 'page' && ($order_meta = $this->getMetaValue('page_order'))) {
			...
			$indexes = array_map(function($p) use ($order) { return array_search($p->ID, $order); }, $posts); 
